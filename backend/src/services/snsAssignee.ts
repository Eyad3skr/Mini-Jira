...
function filterPolicyFor(userId: string): string {
 ...
 */
export async function ensureAssigneeEmailSubscription(
  userId: string,
  email: string
): Promise<void> {
  ...
  const listed = await sns.send(
    new ListSubscriptionsByTopicCommand({ TopicArn: topicArn })
  );

  const existing = listed.Subscriptions?.find(
    (s) => s.Protocol === 'email' && s.Endpoint?.toLowerCase() === endpoint
  );
