...
import {
  deleteTeamAndDependencies,
  getTeamDeletePreview,
  syncTeamDisplayName,
  TeamLifecycleError,
} from '../services/teamLifecycle.js';
import { routeParam } from '../utils/routeParam.js';

const router = Router();

function handleLifecycleError(err: unknown, res: import('express').Response): boolean {
...
router.post('/', authMiddleware, requireRole('admin', 'manager'), async (req, res) => {
  ...
  const teamId = (rawTeamId as string).trim().toLowerCase();
  const name = normalizeTeamName(rawName as string);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (handleLifecycleError(err, res)) return;
      throw err;
    }
  }
);
...
router.patch('/:id', authMiddleware, requireRole('admin', 'manager'), async (req, res) => {
  ...
  const name = normalizeTeamName(req.body.name as string);
